...
/* Board control GPIOs: fixed mapping so DTS does not need gpios on board_controls nodes */
#define BOARD_GPIO_SPEC(port_label, pin_num, flags)                                                \
	{.port = DEVICE_DT_GET(DT_NODELABEL(port_label)), .pin = (pin_num), .dt_flags = (flags)}
...
const struct gpio_dt_spec vext_ctl = BOARD_GPIO_SPEC(gpio0, 21, GPIO_ACTIVE_HIGH);
const struct gpio_dt_spec adc_ctl = BOARD_GPIO_SPEC(gpio0, 6, GPIO_ACTIVE_HIGH);
const struct gpio_dt_spec tft_en = BOARD_GPIO_SPEC(gpio0, 3, GPIO_ACTIVE_LOW);
const struct gpio_dt_spec tft_led_en = BOARD_GPIO_SPEC(gpio0, 15, GPIO_ACTIVE_LOW);
const struct gpio_dt_spec gnss_rst = BOARD_GPIO_SPEC(gpio1, 6, GPIO_ACTIVE_LOW);
const struct gpio_dt_spec gnss_wakeup = BOARD_GPIO_SPEC(gpio1, 2, GPIO_ACTIVE_HIGH);
